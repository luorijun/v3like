...
项目使用运行时生成的无高程单位球面，默认进入海陆归属模式，从 `Content/surface.bin` 读取陆地、海洋和湖泊分类并配色。高程地形模式从 `Content/terrain.bin` 读取每格高度并配色。两种资产随源码和发布包提供，普通运行无需安装 GMT。

数字键 `0` 切换原始模式，`1` 切换海陆归属模式，`2` 切换高程地形模式。
