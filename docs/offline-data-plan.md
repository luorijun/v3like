...
本文记录地形与水文数据的来源、清理方式、游戏数据结构和视觉表达。当前基准为全球球面、约 128 万个游戏地块，以及 Cube-sphere 地形 `ChunkResolution = 17`、`MaximumLod = 7`。
...
1. 最高 LOD 每个 Cube Face 为 `2049 × 2049` 顶点，全球去重后共 25,165,826 个顶点，平均面元边长约 4.5 km。
...
5. Cube Face 共边和角点使用统一球面位置键，只采样和量化一次。
6. 陆地高程量化为 `int16` 米；海洋几何固定为海平面。海深只映射到游戏地块：`≤ 200 m` 为浅海，`> 200 m` 为深海。
...
| 对象 | 几何 | 高度与掩膜 |
| --- | --- | --- |
| 地形 | 每 Chunk `17 × 17` 顶点 | 平滑顶点法线；测试 `64 × 64` 和 `128 × 128` 地形法线 |
...
| `int16` 最高 LOD 高程 | 约 48 MiB |
| 131,070 个 LOD 节点元数据 | 约 5.5～8 MiB |
| 独立 1-bit 顶点水体掩膜 | 最多约 3 MiB |
...
- 重要海峡在地块降采样后仍保持通航连通。
- 在约 15 km 最低相机高度比较无地形法线、`64 × 64` 和 `128 × 128` 三档。只有轮廓仍不足时才增加几何 LOD。
- 通过实际数据分布确定河流过滤阈值、湖泊最小面积、湖泊水域格覆盖阈值和河宽等级。

任一输入数据校验和、重采样规则、量化规则、水体分类规则或构建器版本变化，都必须生成新的世界资产版本。
