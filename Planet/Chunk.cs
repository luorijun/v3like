...
internal sealed class Chunk : IDisposable
{
    ...
    private bool ShouldSplit(in SphereView view)
    {
        ...
        var distanceToBounds = Math.Max(
            0.0001f,
            Vector3.Distance(view.CameraPosition, _boundsCenter) - _boundsRadius);
    ...
    private (Vector3 Center, float Radius) CalculateBounds()
    {
        ...
        var center = _face.Project(middleU, middleV)
            * (1.0f + ProceduralTerrain.MaximumElevation * 0.5f);
    ...
    private VertexBuffer CreateVertexBuffer()
    {
        ...
        for (var y = 0; y < _resolution; y++)
        {
            ...
            for (var x = 0; x < _resolution; x++)
            {
                ...
                vertices[y * _resolution + x] = new TerrainVertex(
                    position,
                    normal,
                    terrain.Color);
            }
