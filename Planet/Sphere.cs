...
internal sealed class Sphere : IDisposable {
    ...
    public void Draw(Matrix view, Matrix projection, in PlanetRenderOptions options) {
        ...
        if (options.ShowSurface) {
            DrawSphereGeometry(_surfaceEffect, BlendState.Opaque, DepthStencilState.Default, _solidRasterizerState);
        }
    ...
    private void DrawSphereGeometry(BasicEffect effect, BlendState blendState, DepthStencilState depthStencilState, RasterizerState rasterizerState) {
    ...
    private void DrawGuideLines() {
        ...
        foreach (var pass in _guideLineEffect.CurrentTechnique.Passes) {
            pass.Apply();
            _graphicsDevice.DrawPrimitives(PrimitiveType.LineList, 0, _guideLinePrimitiveCount);
        }
    }

    private static void ConfigureEffect(BasicEffect effect, Matrix view, Matrix projection) {
    ...
    private static VertexPositionColor[] CreateGuideLineVertices() {
        ...
        var vertexIndex = 0;
        var radius = 1.0f + ProceduralTerrain.MaximumElevation + GuideLineSurfaceOffset;

        AddLatitudeLine(vertices, ref vertexIndex, radius, 0.0f, Color.Gold);
        AddLatitudeLine(vertices, ref vertexIndex, radius, TropicLatitudeDegrees, Color.OrangeRed);
        AddLatitudeLine(vertices, ref vertexIndex, radius, -TropicLatitudeDegrees, Color.OrangeRed);
    ...
    private static void AddLatitudeLine(VertexPositionColor[] vertices, ref int vertexIndex, float radius, float latitudeDegrees, Color color) {
        ...
        for (var segment = 0; segment < GuideLineSegments; segment++) {
            ...
            vertices[vertexIndex++] = new VertexPositionColor(LatitudePoint(horizontalRadius, y, startLongitude), color);
            vertices[vertexIndex++] = new VertexPositionColor(LatitudePoint(horizontalRadius, y, endLongitude), color);
        }
    }

    private static Vector3 LatitudePoint(float horizontalRadius, float y, float longitude) {
        return new Vector3(MathF.Cos(longitude) * horizontalRadius, y, MathF.Sin(longitude) * horizontalRadius);
    }

    private static void AddRotationAxis(VertexPositionColor[] vertices, ref int vertexIndex) {
        var axisLength = RotationAxisHalfLength * 2.0f;
        for (var segment = 0; segment < RotationAxisSegments; segment++) {
            var startY = -RotationAxisHalfLength + axisLength * segment / RotationAxisSegments;
            var endY = -RotationAxisHalfLength + axisLength * (segment + 1) / RotationAxisSegments;
            vertices[vertexIndex++] = new VertexPositionColor(Vector3.UnitY * startY, Color.Cyan);
            vertices[vertexIndex++] = new VertexPositionColor(Vector3.UnitY * endY, Color.Cyan);
        }
    }
}

internal readonly record struct PlanetRenderOptions(bool ShowSurface, bool ShowWireframe, bool ShowGuideLines);

internal readonly record struct SphereView(Vector3 CameraPosition, float VerticalFieldOfView, int ViewportHeight);

internal readonly record struct CubeFaceBasis(Vector3 Normal, Vector3 UAxis, Vector3 VAxis);
